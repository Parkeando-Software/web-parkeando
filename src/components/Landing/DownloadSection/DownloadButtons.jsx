  ...
  const buttons = [
    {
      ...
      bg: "bg-slate-950 dark:bg-slate-50",
      hoverBg: "hover:bg-slate-900 dark:hover:bg-slate-200",
      text: "text-white dark:text-slate-950",
      textBg: "text-white dark:text-slate-950",
    ...
    },
    {
      ...
      text: "text-white dark:text-slate-50",
      textBg: "text-white dark:text-slate-50",
      iconViewBox: "0 0 32 32",
      iconPath:
    ...
    <div className="flex flex-col sm:flex-row items-center gap-6 justify-center mb-16">
      {buttons.map((btn, i) => (
        <a
          ...
          href={btn.href}
          target="_blank"
          className={`group inline-flex items-center justify-center ${btn.bg} ${btn.hoverBg} ${btn.text} px-6 py-4 rounded-xl shadow-lg transform transition-all duration-300 border border-white/20 cursor-pointer w-50 h-16`}
        >
          <svg
            width="32"
            height="32"
            ...
            fill="currentColor"
            className="mr-4 shrink-0 transform transition-transform duration-300 group-hover:scale-110"
          >
            <path d={btn.iconPath} />
          </svg>
          <div className="text-left">
            <div className="text-xs opacity-80">{btn.labelSmall}</div>
            <div className="font-semibold">{btn.labelMain}</div>
          </div>
        </a>
      ))}
    </div>
