...
export default function CancelDeleteRequest() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus("error");
      setMessage("Token de cancelación no proporcionado.");
      return;
    }
    ...
    const cancelRequest = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/account/delete-request/cancel/${token}`,
          { method: "POST" }
        );
        ...
        if (res.ok) {
          setStatus("success");
          setMessage(data?.message || "Tu solicitud de eliminación ha sido cancelada correctamente.");
        } else {
          setStatus("error");
          // Mostrar mensaje de error si viene en JSON
          if (data?.error) setMessage(data.error);
          else if (data?.message) setMessage(data.message);
          else setMessage("No se pudo cancelar la solicitud. El token puede ser inválido o ya haber expirado.");
        }
      } catch (e) {
        setStatus("error");
        setMessage("Error de red al intentar cancelar la solicitud. Por favor, inténtalo de nuevo más tarde.");
      }
    };
          ...
          <Card className={`w-full shadow-xl border-0 ${theme === "dark" ? "bg-slate-800/50 backdrop-blur-sm text-white" : "bg-white/80 backdrop-blur-sm"}`}>
            <CardHeader className="text-center">
              <CardTitle className="text-2xl font-bold">
                {status === "loading" && "Cancelando solicitud..."}
                {status === "success" && "Solicitud cancelada"}
                {status === "error" && "Error al cancelar"}
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {status === "loading" && (
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className={theme === "dark" ? "text-gray-300" : "text-gray-600"}>
                      Estamos procesando la cancelación. Esto puede tardar unos segundos.
                    </p>
                  </div>
                )}
